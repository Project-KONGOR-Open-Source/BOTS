...
function object:SkillBuild()	
	...
	if skills.second:GetLevel() < 1 then
...
object.firstUpBonus = 11
object.ultUpBonus = 100

object.firstUseBonus = 11
...
function object:oncombateventOverride(EventData)
	...
	if EventData.Type == "Ability" then
		...
		if EventData.InflictorName == "Ability_Calamity1" then
...
local function HarassHeroExecuteOverride(botBrain)
	...
	if not bActionTaken and bCanSee then
		...
		local nRange = abilFourth:GetRange() + core.GetExtraRange(unitSelf) + core.GetExtraRange(unitTarget)
		...
		if abilFourth:CanActivate() and nDistanceSq < (nRange * nRange) then
			...
			if bUseFourth then
				bActionTaken = core.OrderAbilityPosition(botBrain, abilFourth, vecTargetPos)
			end
		end
	end
...
BotEcho('finished loading calamity_main')
